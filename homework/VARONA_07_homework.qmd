...
quant_mdl <- bf(log_rt ~ log_frequency)

get_prior(quant_mdl, quantese)

#This suggests to me a mean of 6.5 and sd of 2.5 for the intercept and
#flat for the slope
...
#I got a warning: 210 of 4000 (5.0%) transitions ended with a divergence.

mcmc_plot(model_quant, type = "trace")

#I believe some of these caterpillars do not look that healthy? #or well, they look like healthy caterpillars, but some are closer to the floor?
...
model_quant |> 
  ...
  ggplot(aes(x = b_log_frequency)) +
    geom_density(fill = "#43a2ca") +
  stat_halfeye(.width = c(0.95)) +
  labs(x = "b_log_frequency",
       y = "Posterior density")

#We see that the most likely value is -0.06, so for each increase of log_frequency, the log_rt decreases by 0.06 log.
